...
function validateData($data)
{
	...
	if ($validations!=0 and $active!=1) {
		...
		$save=mysqli_query($con, "select * from sellers_tb where  phone='$email' and password='$password' and validation_code=0 and active=1 or email='$email' and password='$password' and validation_code=0 and active=1; ");
	...
	if($log){
		...
		$app=mysqli_query($con, "select * from sellers_tb where phone = '$email' or email = '$email'");
