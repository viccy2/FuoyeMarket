            ...
            <li class="nav-item dropdown no-arrow d-sm-none">
              ...
              <div class="dropdown-menu dropdown-menu-right p-3 shadow animated--grow-in animated zoomIn" aria-labelledby="searchDropdown">
                ...
                <form class="form-inline mr-auto w-100 navbar-search">
                  <div class="input-group">
                    <input type="text" class="form-control bg-light border-0 small searchnow" placeholder="Search for all Company"  onkeyup="searchAll(this.value)" aria-label="Search" aria-describedby="basic-addon2" id="search2">
 ...
 <div class="modal fade" id="addproductModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true " style="margin-top:6pc;">
    ...
    <div class="modal-dialog" role="document">
      <div class="modal-content" style="background:<?php echo $_SESSION['bodybgcolor']; ?>;color:<?php echo $_SESSION['color']; ?>;">
        <div class="modal-header">
          <h5 class="modal-title" id="exampleModalLabel">Want to delete product account?</h5>
          <button class="close" type="button" data-dismiss="modal" aria-label="Close">
            <span aria-hidden="true">×</span>
          </button>
        </div>
        ...
        <button class="btn text-gray-600" onclick="deleteCompany()" id="leave_group">Delete</button>
    ...
    function showPreview(objFileInput) 
{
 ...
 <div class="modal fade" id="productsModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true " style="margin-top:3pc;">
    <div class="modal-dialog" role="document">
      <div class="modal-content" style="">
        <div class="modal-header">
        ...
        <div class="modal-body">
          <form id="pdetails" method="post" action="uploadproduct">
          <input type="text" name="cname" value="" id='addDetq' hidden>
          <div class="input-field">
            <i class="fa fa-glob"></i>
            <input type="text" name="name" placeholder="Product name" required="required" id="">
          </div>
          <div class="input-field">
            <i class="fa fa-naira"></i>
            <input type="number" name="price" placeholder="Product price" required="required" id="">
          </div>
           <center>
           ...
           <div class="">
            ...
            <input type="file" class="inputfile" name="fileToUpload" id="your_picturs"  onchange="showPreview(this);" data-multiple-caption="{count} files selected" hidden />
            <label for="your_picturs">
            <figure id="meme-bg-target">
        ...
        <div class="modal-footer">
        <button class="btn" data-dismiss="modal">Cancel</button>
        <button class="btn" href="#" onclick="sendproductDetails()">Save changes</button>
...
<script>
     ...
     $(document).ready(function(){
        var form = $("#pdetails");
        var formData = new FormData($("#pdetails")[0]);
 ...
 <script>
  ...
  <script>
    function searchAll(s)
          {
          c = s;
          $.post("product_search.php",{first:c},function(response) {
          var arr = JSON.parse(response);
          var i;
          var out = "";
          for(i = 0; i < arr.length; i++) {
          out += "<a><div class='chip animated zoomIn text-gray-600 ' style='color:#673AB7;width:100%;font-size:14px;background:rgba(0, 0, 0.2, 0.1); '><img src='"+arr[i].display_pic+"' alt='Person' width='96' height='96' data-target='#picModal' data-toggle='modal' onclick='pic("+arr[i].product_id+" )' style='cursor:pointer'>"+arr[i].company_name+" <div style='float:right'><button onclick='edit("+arr[i].product_id+" ) ' class='btn pd-setting-ed' data-target='#editModal' data-toggle='modal'><i class='fa fa-edit fa fa-sm' aria-hidden='true'></i></button><button onclick='add("+arr[i].product_id+" ) ' class='btn pd-setting-ed' ' data-target='#companyDetailsModal' data-toggle='modal'><i class='fa fa-plus fa fa-sm' aria-hidden='true'></i></button> <button onclick='deletes("+arr[i].product_id+")'; data-target='#deleteModal' data-toggle='modal' class='btn pd-setting-ed' style='float:right;height:50px'><i class='fa fa-trash fa fa-sm' aria-hidden='true'></i></button><button onclick='done("+arr[i].product_id+" ) ' class='btn pd-setting-ed' ' data-target='#productsModal' data-toggle='modal'><i class='fa fa-shopping-cart fa fa-sm' aria-hidden='true'></i></button></div></div><br><br></a>";
...
<script>
    ...
    // alert(e);
     ...
     c = e;
          $.post("details2.php",{first:c},function(response) {
          var arr = JSON.parse(response);
          var i;
          var out = "";
          for(i = 0; i < arr.length; i++) {
            document.getElementById('addDetq').value=arr[i].company_name;
        ...
        var formData = new FormData($("#deleteform")[0]);
          ...
          $.ajax({
              ...
              success: function(data){
                ...
                $('#error').text(data);
                var b = data;
               var match = data.match("account deleted");
               var matchs = data.match("error");
...
<!--=======================================AJAXFORM=============================================-->
 ...
 <form action="delete_company.php" method="post" id="deleteform" hidden>
  <input type="text" name="id" id="del" hidden>
</form>
