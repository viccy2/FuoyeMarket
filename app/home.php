<!DOCTYPE html>
<html>
<head>
	<title>FuoyeMarket, - Connect with your choice sellers.</title>
	<meta charset="utf-8">
    <meta name="viewport" content="width=device-width" initial-scale="1">
     <!--============================STYLES=====================================================-->
   <link rel="icon" type="image/x-icon" href="../images/fm.jpeg" >
	<link rel="stylesheet" type="text/css" href="../css/sb-admin-2.css">
	<link rel="stylesheet" type="text/css" href="../css/animate.css">
	<link rel="stylesheet" type="text/css" href="../css/sliders.css">
	<link rel="stylesheet" type="text/css" href="../css/preloader.css">
  <link rel="stylesheet" type="text/css" href="style.css">
	<link rel="stylesheet" type="text/css" href="../font-awesome/css/font-awesome.css">
	<script src="../js/jquery.3.2.1.min.js"></script>
	<script src="../js/bootstrap.min.js"></script>

	
</head>
<body onload="searchAll('');" style="width:100%;">
	 <!--============================HEADER=====================================================-->
<header>
    <!-- Content Wrapper -->
    <div id="content-wrapper" class="d-flex flex-column">

      <!-- Main Content -->
      <div id="content">

        <!-- Topbar -->
        <nav class="navbar navbar-expand navbar-light  topbar mb-4 fixed-top" style="background:white;width:100%">

          <!-- Sidebar Toggle (Topbar) -->
          <a href=""> <img src="../images/fm.jpeg" style="height:50px; width:50px;border-radius:50%"> </a>

         <a href=""> <h5 class="text-gray-100 mt-3" style="cursor:pointer;"><b></b></h5></a>

          <!-- Topbar Search -->
          

          <!-- Topbar Navbar -->
          <ul class="navbar-nav ml-auto">
 <!-- Nav Item - User Information -->
           <!--  <li class="nav-item dropdown no-arrow">
              <a class="nav-link dropdown-toggle" href="account" id="" role="button" >
                <span class="mr-2  d-lg-inline text-gray-300 small">New Account</span>
                
              </a>
              
            </li> -->
             <!-- Nav Item - User Information -->
           <!--  <li class="nav-item dropdown no-arrow">
              <a class="nav-link dropdown-toggle" href="account" id="" role="button" >
                <span class="mr-2  d-lg-inline text-gray-300 small">New Account</span>
                
              </a>
              
            </li> -->
            <!-- Nav Item - Search Dropdown (Visible Only XS) -->
            <li class="nav-item dropdown no-arrow ">
               <form class=" p-3 ">
            <div class="input-group">
           <!--   <select class="form-control" name="product_category" id="categorys" required onselect="searchAll(this.value)" style="width:120px">
                <option value="" selected disabled>Category</option>
                <option value="Food/Beverages">Food/Beverages</option>
                <option value="Phones/Gadgets/Smart Watches">Phones/Gadgets/Smart Watches</option>
                <option value="Jewelries/Accessories">Jewelries/Accessories</option>
                <option value="Cosmetics/Make-up">Cosmetics/Make-up</option>
                <option value="Fashion Designing">Fashion Designing</option>
                <option value="Barbing/Hair-styling">Barbing/Hair-styling</option>
                <option value="Wears/Clothes/Shoes">Wears/Clothes/Shoes</option>
                <option value="Bags/Luggages">Bags/Luggages</option>
                <option value="Crypto Trading">Crypto Trading</option>
                <option value="Electronics">Electronics</option>
                <option value="Books/Stationaries">Books/Stationaries</option>
                <option value="Computer/Aplliances">Computer/Aplliances</option>
                <option value="Games/Gaming">Games/Gaming</option>
                <option value="Sporting Goods">Sporting Goods</option>
                <option value="Home/Office">Home/Office</option>
                 <option value="Automobile">Automobile</option>
                 <option value="Exchange Service">Exchange Service</option>

                
              </select> -->
             
            </div>
          </form>
        
            </li>


            <!-- Nav Item - Alerts -->
          

            <!-- Nav Item - Messages -->


            <div class="topbar-divider d-none d-sm-block"></div>

            <!-- Nav Item - User Information -->
            <li class="nav-item dropdown no-arrow">
              <a class="nav-link dropdown-toggle" href="account" id="" role="button" >
                <span class="mr-2  d-lg-inline small" style="color:#673AB7">Create Account</span>
                
              </a>
              
            </li>

          </ul>

        </nav>
</div>
</div>
</header>
 <!--============================</>=====================================================-->
  <!--============================APP=====================================================-->
<div class="payment-cart-pro  ">
            <div class="container-fluid text-gray-600">
            <div class="row" id="displaybox" >

</div></div></div>
  

 

  <div class="app" id="app">
  
              
 <!--============================SLIDER=====================================================-->
	<div class="hero_area" style="margin-top:;">
    <div class="hero_bg_box" style="height:600px; width:100%;">
      <img src="../images/29.jpg" alt="" style="">
    </div>
    <!-- header section strats -->
    <header class="header_section">
    
    </header>
    <!-- end header section -->
    <!-- slider section -->
    <section class="slider_section animated zoomIn">
      <div id="customCarousel1" class="carousel slide" data-ride="carousel">
        <div class="carousel-inner" style="margin-top:-50px">
          <div class="carousel-item active">
            <div class="container">
              <div class="row">
                <div class="col-md-6">
                  <div class="detail-box">
                   
                    <h2 style="color: ; font-weight:200px; font-size:40px">
                      Connect <br />
                      with Sellers around you...
                    </h2>
                    <a href="#" class="" style="background: #673AB7">
                      Search now
                    </a>
                  </div>
                </div>
              </div>
            </div>
          </div>
          <div class="carousel-item ">
            <div class="container">
              <div class="row">
                <div class="col-md-6">
                  <div class="detail-box">
                    <h2 style="color: ; font-weight:200px; font-size:40px">
                      Unlimited <br />
                      access to choice products...
                    </h2>
                    <a href="#" class="" style="background:#673AB7">
                      Search now
                    </a>
                  </div>
                </div>
              </div>
            </div>
          </div>
           <div class="carousel-item ">
            <div class="container">
              <div class="row">
                <div class="col-md-6">
                  <div class="detail-box">
                    <h2 style="color: ; font-weight:200px; font-size:40px">
                      Upload <br />
                      your products...
                    </h2>
                    <a href="account" class="" style="background: #673AB7">
                      Create Account
                    </a>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
       <!--  <ol class="carousel-indicators">
          <li data-target="#customCarousel1" data-slide-to="0" class="active"></li>
          <li data-target="#customCarousel1" data-slide-to="1"></li>
          <li data-target="#customCarousel1" data-slide-to="2"></li>
        </ol> -->
      </div>
    </section>
    <!-- end slider section -->
  </div>

 <!--============================</>=====================================================-->
<!-- veg section -->

  <section class="veg_section layout_padding  " style="margin-top:-220px;">
    <div class="container">
      <div class="heading_container heading_center">
      <br><br> <br><br>
      </div>
      <div class="row">
        <div class="col-md-6">
          <div class="box">
           <!--  <div class="img-box">
              <img src="../images/f.png" alt="">
            </div> -->
            <div class="detail-box">
              <a href="">
              
              </a>
              <div class="price_box animated slideInLeft"><br>
                  <h3 style="color: #673AB7; font-weight:200px; font-size:33px">
            Grow, Connect, Sell...
          </h3>
               <p class="price_heading text-left container-fluid text-gray-600" style="">
                 FuoyeMarket is a platform created for the main pupose of connecting campus students to the sellers around them. The platform created for best access to trading. FuoyeMarket can boast of it's features of getting products available to buyers.

                </p>
                 <p class="price_heading text-left container-fluid text-gray-600" style="">
                 FuoyeMarket gives the opportunity for sellers based on variety of services, products and goods to render or sell to create sellers account and upload their products to potential buyers. Also, for users to create account and have medium to add their products and connect with the sellers.

                </p>
              </div>
            </div>
          </div>
        </div>
        
        <div class="col-md-6 ">
          <div class="box animated zoomIn ">
            <!-- <div class="img-box">
              <img src="../images/d.png" alt="">
            </div> -->
            <div class="detail-box">
              <a href="">
               
              </a>
              <div class="price_box">
                <br><br>
             
                <img src="../images/30.png" style="border-radius:15%;height:200px;width:100%">
         
              </div>
            </div>
          </div>
        </div>
       
      
       
        
       
        
      </div>
      
    </div>
  </section>

  <!-- end veg section -->

 <section class="veg_section layout_padding animated zoomIn " style="margin-top:-100px;">
    <div class="container">
     <div class="row">
      <div class="col-md-6">
        <div class="card-body">
          <h3 style="color:#673AB7">Our Aims & Objectives</h3>
          <p class="text-gray-600">To create an online presence for sellers to connect with their prospective buyers.
            Also, to connect campus students with their choice sellers around them.
           </p>
          
        </div>
      </div>
       <div class="col-md-6">
        <div class="card-body">
           <img src="../images/k2.jpg">
        </div>
      </div>
       
     </div>
      
    </div>
  </section>


<!-- veg section -->

  

  <!-- end veg section -->




  <!-- about section -->

  <section class="about_section animated zoomIn">
    <div class="about_bg_box">
      <img src="../images/b.jpg" alt="">
    </div>
    <div class="container ">
      <div class="row">
        <div class="col-md-6 ml-auto ">
          <div class="detail-box animated zoomIn">
            <div class="heading_container">
              <h2>
                Get access to  <br>
                Best sellers in the business.
              </h2>
            </div>
            <p>
              Search for the best sellers on <b>FuoyeMarket</b>, get the access to sellers around you for your needed products...

            </p>
            
          </div>
        </div>
      </div>
    </div>
  </section><br><br>

  <!-- end about section -->
 <!--============================</>=====================================================-->
<!--============================</>=====================================================-->

<!-- veg section -->

 

  <!-- end veg section -->



  <!-- client section -->

  <section class="client_section layout_padding-bottom " >

    <div class="container">
      <div class="heading_container heading_center"><br><br><br>
        <h3 style="color:#673AB7; font-weight:200px; font-size:33px ">
         Don’t just take our words for it
        </h3>
      </div>
      <div class="row">
      <div class="col-md-6">
        <div class="box">
    
                <img src="../images/i.png" alt="" style="height:180px;width:100%;border-radius:0%">
              </div>
              <div class="name">
                <h5 class="text-gray-700 text-center">
                  Ayomide
                </h5>
                <h6 class="text-gray-700 text-center">
                 User
                </h6>
              </div>
            </div>
          
      
      <div class="col-md-6">
        <div class="box text-gray-700">
           FuoyeMarket is an awesome platform, I love using it because all sellers on the platform are trusted sellers, a good platform to connect with my choice sellers.
        </div>
      </div>
    </div>
     <div class="row">
      <div class="col-md-6">
        <div class="box">
    
                <img src="../images/pel.jpg" alt="" style="height:180px;width:100%;border-radius:;">
              </div>
              <div class="name">
                <h5 class="text-gray-700 text-center">
               PelvixBuy'N'Sell
                </h5>
                <h6 class="text-gray-700 text-center">
                 Seller
                </h6>
              </div>
            </div>
          
      
      <div class="col-md-6">
        <div class="box text-gray-700">
           FuoyeMarket is a great platform, easy to use and very awesome. It allows me to have audience for my goods. Indeed a great platform.
        </div>
      </div>
    </div>

        
      
    </div>
  </section>

  <!-- end client section -->
  
<!-- veg section -->

  <section class="veg_section layout_padding animated zoomIn " style="margin-top:-200px;">
    <div class="container">
      <div class="heading_container heading_center"><br><br><br>
        <h3 style="color: #673AB7; font-weight:200px; font-size:33px ">
          Our Partners
        </h3><br>
      </div>
      <div class="row">
        <div class="col-md-6 col-lg-4">
          <div class="box">
            <div class="img-box">
              <img src="../images/pel.jpg" alt="" style="border-radius:50%;height:100px;width:100px;">
            </div>
            <div class="detail-box" style="margin-top:-60px">
              <a href="https://wa.me/+2348146828289" style="color: #673AB7; ">
               PELVIXBUY'N'SELL
              </a>
             
            </div>
             <div class="price_box" style="height:50px">
                
              </div>
          </div>
        </div>
       
        <div class="col-md-6 col-lg-4" style="margin-top:0px;">
          <div class="box">
            <div class="img-box">
              <img src="https://doteightplus.com/images/dot.svg" alt="" style="border-radius:50%;height:200px;height:100px;width:100px;">
            </div>
            <div class="detail-box" style="margin-top:-60px;">
              <a href="" style="color: #673AB7; ">
                DOTEIGHTPLUS
              </a>
             
            </div>
             <div class="price_box" style="height:50px">
                
              </div>
          </div>
        </div>
      
       
         <div class="col-md-6 col-lg-4">
          <div class="box">
            <div class="img-box">
              <img src="https://dotpedia.com.ng/images/cover.png" alt="" style="border-radius:50%;height:200px;height:100px;width:100px;">
            </div>
            <div class="detail-box" style="margin-top:-50px;">
               <a href="https://dotpedia.com.ng/" style="color: #673AB7;">
               DOTPEDIA
              </a>
             
            </div>
             <div class="price_box" style="height:50px">
                
              </div>
          </div>
        </div>
       
      
       
        
       
        
      </div>
      
    </div>
  </section>
<br><br><br><br>
  <!-- end veg section -->

<!-- footer section -->
  <section class="container-fluid footer_section animated zoomIn">
    <div class="container">
      <div class="row ">
        <div class="col-sm-6 col-md-4 col-lg-3 footer-col">
          <div class="footer_detail">
            <a href="index.html">
              <h4>
               FuoyeMarket
              </h4>
            </a>
            <p>
              Follow us on our social platforms.
            </p>
            <div class="social_box">
              <a href="#">
                <i class="fa fa-facebook" aria-hidden="true"></i>
              </a>
           
              <a href="https://wa.me/+2349072045997">
                <i class="fa fa-whatsapp" aria-hidden="true"></i>
              </a> 

              <a href="https://instagram.com/fuoye_market?utm_medium=copy_link">
                <i class="fa fa-instagram" aria-hidden="true"></i>
              </a>
            </div>
          </div>
        </div>
        <div class="col-sm-6 col-md-4 col-lg-3 mx-auto footer-col">
          
        <div class="contact_nav">
            
        </div>
        </div>
        <div class="col-md-4 footer-col">
          <div class="footer_form">
           <h4>
            Contact us
          </h4>
          <p>
            Get in touch with us... 
          </p>
          <div class="contact_nav">
            
            <a href="">
              <i class="fa fa-phone" aria-hidden="true"></i>
              <span>
                Call : +234 9072045997
              </span>
            </a>
            <a href="">
              <i class="fa fa-envelope" aria-hidden="true"></i>
              <span>
                Email : fuoyemarket@gmail.com
              </span>
            </a>
           
          </div>
        </div>
      </div>
      
    </div>
  </section>
  <!-- end  footer section -->
 <!--============================</>=====================================================-->

</div>

  

















  <!--============================FOOTER=====================================================-->
 <footer class=" bg-white fixed-bottom" style="height:30px;font-size:15px;color:#673AB7;">
        <div class="container my-auto">
          <div class="copyright text-center my-auto">
            <span ><b>FuoyeMarket</b> &copy;  2021  <b></b></span>
           
          </div>
        </div>
      </footer>
  <!--============================SCRIPTS=====================================================-->

    <script>
      $(document).ready(function(){
        $('#categorys').change(function(){
          var all = $(this).val();
          // alert(all);
   
      $("#displaybox").show();
      $("#app").hide();
          
          c = all;
          $.post("company_search.php",{first:c},function(response) {
          var arr = JSON.parse(response);
          var i;
          var out = "";
          for(i = 0; i < arr.length; i++) {
          out += " <div class='col-lg-4 col-md-4 col-sm-4 col-xs-12' style='margin-top: 100px'><div class='card payment-card responsive-mg-b-30 card border-left-primary shadow h-100 py-2'><div class='payment-inner-pro'><i class='fa fa-cc-paypa' aria-hidden='true'><img src='"+arr[i].display_pic+"' alt='Person' width='96' height='96' style='width:40px;height:40px;border-radius: 50%'></i><h5>"+arr[i].company_name+"</h5><div class='row m-t-10'><div class='col-sm-6'><strong class='m-r-5'>Product Category : </strong>"+arr[i].product_category+"<br> <strong class='m-r-5'>Location : </strong>"+arr[i].location+"</div><div class='col-sm-6 text-right'> <a href='contact?user="+arr[i].seller_id+"&category="+arr[i].product_category+"&cmpn="+arr[i].company_name+"'><button class='btn'>Contact</button></a></div></div></div></div><br><br></div>";

            }
            $('#displaybox').html(out);
            });
            
          });
          });
</script>


 <script src="../js/notifications/Lobibox.js"></script>
 <script src="../js/sb-admin-2.js"></script>

 
</body>
</html>